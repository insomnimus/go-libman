...
func resetConfig() {
	...
	configHome := userdir.LibmanConfigDir()
...
func resetDB() {
	...
	input := cmd.PromptNormal()
	dbPath := userdir.LibmanDBDir() + "/libman.db"
	switch strings.ToLower(input) {
	case "y", "yes", "yea":
